        ...
        <SafeAreaView className="flex-1 bg-white" edges={['top']}>
            ...
            <View className="flex-row items-center px-4 py-2 bg-gray-50 border-b border-gray-100">
                <ScrollView horizontal showsHorizontalScrollIndicator={false} className="flex-1">
                    <TouchableOpacity
                        ...
                        className={`px-4 py-2 rounded-full mr-2 border ${sortBy === 'default' ? 'bg-indigo-600 border-indigo-600' : 'bg-white border-gray-200'}`}
                    >
                        <Text className={`text-xs font-bold ${sortBy === 'default' ? 'text-white' : 'text-gray-600'}`}>Recommended</Text>
                    </TouchableOpacity>
                    <TouchableOpacity
                        onPress={() => setSortBy('price_low')}
                        className={`px-4 py-2 rounded-full mr-2 border ${sortBy === 'price_low' ? 'bg-indigo-600 border-indigo-600' : 'bg-white border-gray-200'}`}
                    >
                        <Text className={`text-xs font-bold ${sortBy === 'price_low' ? 'text-white' : 'text-gray-600'}`}>Price: Low to High</Text>
                    </TouchableOpacity>
                    <TouchableOpacity
                        onPress={() => setSortBy('price_high')}
                        className={`px-4 py-2 rounded-full mr-2 border ${sortBy === 'price_high' ? 'bg-indigo-600 border-indigo-600' : 'bg-white border-gray-200'}`}
                    >
            ...
            {loading && !refreshing ? (
                <View className="flex-1 justify-center items-center">
                    ...
                    <Text className="mt-4 text-gray-500 font-medium">Fetching brand items...</Text>
                </View>
            ) : (
                <FlatList
                    data={sortedProducts()}
                    ...
                    ListEmptyComponent={
                        <View className="flex-1 items-center justify-center pt-20">
                            <Feather name="tag" size={64} color="#D1D5DB" />
