...
const CartScreen = () => {
  ...
  const renderItem = ({ item }: { item: any }) => (
    <View className="bg-white p-4 mb-4 rounded-xl flex-row items-center shadow-sm mx-4 mt-2">
      <Image
        source={{
          uri: item.image || item.image_url || "https://via.placeholder.com/80",
        }}
      ...
      <View className="flex-1 ml-4 justify-between h-20">
        ...
        <View className="flex-row justify-between items-center mt-2">
          <Text className="font-bold text-blue-600 text-lg">₹{item.price}</Text>
  ...
  if (cartItems.length === 0) {
    return (
      <SafeAreaView
        edges={["top"]}
        className="flex-1 bg-gray-50 items-center justify-center p-6"
      >
