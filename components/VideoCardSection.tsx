...
const VideoCardSection = () => {
  ...
  const getYouTubeEmbedUrl = (url: string) => {
    ...
    return match && match[2].length === 11
      ? `https://www.youtube.com/embed/${match[2]}?playsinline=1&controls=0`
      : null;
  };
  ...
  return (
    <View className="py-6 bg-white w-full">
      {/* Left Section: Video */}
      {currentVideo && (
        <View className="w-full h-[220px] bg-black mb-4 relative">
          {currentVideo.video_url ? (
            isYouTubeUrl(currentVideo.video_url) ? (
              <WebView
                source={{
                  uri: getYouTubeEmbedUrl(currentVideo.video_url) || "",
                }}
                style={{ flex: 1 }}
