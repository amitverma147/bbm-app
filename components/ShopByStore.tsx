...
const ShopByStore = ({ sectionName, sectionDescription }: ShopByStoreProps) => {
    ...
    return (
        <View className="py-6 px-4">
            <View className="mb-4">
                <Text className="text-2xl font-black text-center text-orange-600 shadow-sm">{sectionName || "Shop by Store"}</Text>
                {sectionDescription && <Text className="text-center text-gray-500 text-xs mt-1">{sectionDescription}</Text>}
            </View>
            ...
            <View className="flex-row flex-wrap justify-between">
                {stores.map((store) => (
                    ...
                    >
                        <View className="w-16 h-16 sm:w-20 sm:h-20 bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden mb-2 p-1 relative">
                            {store.image_url ? (
                                <Image
                        ...
                        <Text className="text-xs font-semibold text-center text-gray-800" numberOfLines={1}>{store.name}</Text>
                    </TouchableOpacity>
                ))}
            </View>
        </View>
