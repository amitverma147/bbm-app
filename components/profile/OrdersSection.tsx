...
import { View, Text, TouchableOpacity, ActivityIndicator, ScrollView, Image, Alert, TextInput, Pressable } from "react-native";
...
const OrdersSection = () => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                if (result.success && result.order) {
                    ...
                    // Map items for potential returns
    ...
    const handleCheckEligibility = async () => {
        ...
        try {
            const response = await fetch(`${API_BASE_URL}/return-orders/eligibility/${selectedOrder.id}`);
            const data = await response.json();
            if (data.success) {
                setEligibility(data.eligibility);
                if (data.eligibility.can_cancel && !data.eligibility.can_return) {
                    setReturnForm((p) => ({ ...p, return_type: "cancellation" }));
                } else if (data.eligibility.can_return && !data.eligibility.can_cancel) {
                    setReturnForm((p) => ({ ...p, return_type: "return" }));
                }
                setShowReturnForm(true);
            } else {
                Alert.alert("Not Eligible", data.error || "This order is not eligible for return/cancellation.");
            }
        } catch (err) {
    ...
    const toggleItemSelection = (itemId: string) => {
        setOrderItems((prev) => prev.map((i) => i.order_item_id === itemId ? { ...i, selected: !i.selected } : i));
    };

    const submitReturn = async () => {
        const itemsToReturn = orderItems
            .filter((i) => i.selected)
            .map((i) => ({ order_item_id: i.order_item_id, quantity: i.return_quantity, reason: returnForm.reason }));

        if (returnForm.return_type === "return" && itemsToReturn.length === 0) {
        ...
        try {
            const response = await fetch(`${API_BASE_URL}/return-orders/create`, {
                ...
                body: JSON.stringify({
                    ...
                    ...returnForm,
                    items: itemsToReturn.length > 0 ? itemsToReturn : undefined
                }),
            });
            const data = await response.json();
            if (data.success) {
                ...
                setEligibility(null);
                setReturnForm({
                    return_type: "return", reason: "", additional_details: "",
    ...
    // Order Details View (with conditional return form)
    if (selectedOrder) {
        return (
            <NavigationIndependentTree>
                <NavigationContainer>
                    <View className="bg-white flex-1 p-4 rounded-xl shadow-sm border border-gray-100">
                        <View className="flex-row items-center border-b border-gray-100 pb-4 mb-4">
                            ...
                            <View className="flex-1 ml-2">
                                <Text className="text-lg font-bold text-gray-900">
                                    {showReturnForm ? "Request Return/Cancel" : `Order #${String(selectedOrder.id).slice(-8).toUpperCase()}`}
                                </Text>
                        ...
                        {loadingOrderDetails || isCheckingEligibility ? (
                            <View className="py-12 items-center justify-center">
                                <ActivityIndicator size="large" color="#FF6B00" />
                            </View>
                        ) : showReturnForm && eligibility ? (
                            <ScrollView showsVerticalScrollIndicator={false}>
                                <View className={`p-4 rounded-lg mb-4 border ${eligibility.can_return || eligibility.can_cancel ? 'bg-green-50 border-green-200' : 'bg-red-50 border-red-200'}`}>
                                    <Text className="font-bold text-gray-900 mb-1">
                                        {eligibility.can_return || eligibility.can_cancel ? "Eligible" : "Not Eligible"}
                                    </Text>
                                    <Text className="text-xs text-gray-700">{eligibility.reason}</Text>
                                </View>

                                {(eligibility.can_return || eligibility.can_cancel) && (
                                    <View>
                                        {returnForm.return_type === 'return' && orderItems.length > 0 && (
                                            <View className="mb-4">
                                                <Text className="font-semibold mb-2 text-gray-800">Select Items to Return</Text>
                                                {orderItems.map((item) => (
                                                    <TouchableOpacity
                                                        key={item.order_item_id}
                                                        onPress={() => toggleItemSelection(item.order_item_id)}
                                                        className={`p-3 rounded-lg border mb-2 flex-row items-center justify-between ${item.selected ? 'border-[#FF6B00] bg-orange-50' : 'border-gray-200'}`}
                                                    >
                                                        <Text className="font-medium text-sm flex-1">{item.product_name}</Text>
                                                        <Ionicons name={item.selected ? 'checkbox' : 'square-outline'} size={24} color={item.selected ? '#FF6B00' : '#d1d5db'} />
                                                    </TouchableOpacity>
                                                ))}
                                            </View>
                                        )}

                                        <Text className="font-semibold text-sm mb-1 text-gray-800">Reason</Text>
                                        <TextInput
                                            value={returnForm.reason}
                                            onChangeText={(t) => setReturnForm({ ...returnForm, reason: t })}
                                            placeholder="Why are you returning this?"
                                            className="border border-gray-300 rounded-lg p-3 mb-4 bg-gray-50 text-sm"
                                        />

                                        <Text className="font-semibold text-sm mb-1 mt-2 text-gray-800">Bank Details (Optional for Wallet Refund)</Text>
                                        <TextInput
                                            value={returnForm.bank_account_number}
                                            onChangeText={(t) => setReturnForm({ ...returnForm, bank_account_number: t })}
                                            placeholder="Account Number"
                                            className="border border-gray-300 rounded-lg p-3 mb-2 bg-gray-50 text-sm"
                                        />
                                        <TextInput
                                            value={returnForm.bank_ifsc_code}
                                            onChangeText={(t) => setReturnForm({ ...returnForm, bank_ifsc_code: t })}
                                            placeholder="IFSC Code"
                                            className="border border-gray-300 rounded-lg p-3 mb-4 bg-gray-50 text-sm"
                                        />

                                        <TouchableOpacity onPress={submitReturn} disabled={submittingReturn} className={`py-4 rounded-xl items-center mt-2 ${submittingReturn ? 'bg-orange-300' : 'bg-[#FF6B00]'}`}>
                                            {submittingReturn ? <ActivityIndicator size="small" color="white" /> : <Text className="text-white font-bold text-base">Submit Request</Text>}
                                        </TouchableOpacity>
                                    </View>
                                )}
                            </ScrollView>
    ...
    // Order List View & Returns List View
    return (
        <NavigationIndependentTree>
            <NavigationContainer>
                <View className="flex-1">
                    ...
                    {activeTab === "history" && (
                        orders.length > 0 ? (
                            ...
                            <View className="flex-1 items-center justify-center p-8">
                                ...
                                <Text className="text-gray-500 text-center mt-2">Looks like you haven't made your menu order yet.</Text>
                            </View>
                        )
                    )}
                        ...
                        <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 16 }}>
                            {returnRequests.length === 0 && refundRequests.length === 0 && (
                                <View className="items-center justify-center py-10">
                                    ...
                                    <Text className="text-gray-500 font-medium mt-4">No returns or refunds initiated</Text>
