...
const ProductCard = ({ item, onAdd, onPress }: ProductCardProps) => {
    ...
    useEffect(() => {
        if (item.variants && item.variants.length > 0) {
            ...
            const defaultVar = item.variants.find((v: any) => v.is_default) || item.variants[0];
    ...
    const blurhash = '|rF?hV%2WCj[ayj[a|j[az_NaeWBj@ayfRayfQfQfQfQfQfQfQfQfQfQfQfQfQfQfQfQfQfQfQfQfQfQfQfQfQfQfQfQfQfQfQfQfQfQfQfQfQfQfQfQfQfQfQfQfQfQfQfQfQfQfQfQfQfQfQfQfQfQfQfQ';

    // Determine display data based on selection or fallback to item
    const displayPrice = selectedVariant ? (selectedVariant.price || selectedVariant.variant_price) : item.price;
    const displayOldPrice = selectedVariant ? (selectedVariant.old_price || selectedVariant.variant_old_price) : item.oldPrice;
    const displayWeight = selectedVariant ? (selectedVariant.title || selectedVariant.variant_weight) : item.weight;
        ...
        <>
            ...
            >
                ...
                <View className="p-3">
                        ...
                        <View className="flex-row items-center gap-1 mb-1 flex-wrap">
                            {item.brand && (
                                <View className="bg-blue-50 border border-blue-100 px-1.5 py-0.5 rounded">
                                    <Text className="text-[8px] font-medium text-blue-700">{item.brand}</Text>
                                </View>
                            )}
                            {item.store && (
                                <View className="bg-purple-50 border border-purple-100 px-1.5 py-0.5 rounded">
                                    <Text className="text-[8px] font-medium text-purple-700">{item.store}</Text>
                                </View>
                            )}
                        </View>
                    )}

                    <Text className="text-xs font-bold text-gray-800 leading-4 mb-1 h-8" numberOfLines={2}>
                        {item.name}
                    </Text>
                    ...
                    >
                        <Text className={`text-[10px] font-medium ${showVariants ? 'text-[#FD5B00]' : 'text-gray-500'}`}>
                            {displayWeight}
                        </Text>
                        {hasVariants && (
                            <MaterialIcons
                                ...
                                style={{ transform: [{ rotate: showVariants ? '180deg' : '0deg' }] }}
                            />
                        )}
                    </TouchableOpacity>
                    ...
                    <View className="flex-row items-end justify-between mt-auto">
                        ...
                        {quantity > 0 ? (
                            <View className="flex-row items-center bg-[#FF6B00] rounded-lg h-7 px-1 shadow-sm">
                                <TouchableOpacity onPress={handleDecrement} className="w-6 items-center justify-center">
                                    <Text className="text-white font-bold text-lg">-</Text>
                                </TouchableOpacity>
                                <Text className="text-white font-bold text-xs w-4 text-center">{quantity}</Text>
                                <TouchableOpacity onPress={handleIncrement} className="w-6 items-center justify-center">
                                    <Text className="text-white font-bold text-lg">+</Text>
                                </TouchableOpacity>
                            </View>
                        ) : (
                            <TouchableOpacity
                                onPress={() => handleAdd()}
                                className="bg-white border border-[#FF6B00] bg-orange-50 px-4 py-1.5 rounded-lg shadow-sm"
                            >
                                <Text className="text-[#FF6B00] text-[10px] font-bold uppercase">Add</Text>
                            </TouchableOpacity>
                        )}
                    </View>
                    ...
                    {item.stockInfo?.low_stock && (
                        <View className="flex-row items-center mt-1">
                            ...
                            <Text className="text-[8px] text-orange-600 font-medium">Only {item.stockInfo.available_stock} left</Text>
                        </View>
                    )}
                </View>
            </TouchableOpacity>
            ...
            >
                <View className="flex-1 bg-black/50 justify-end">
                    <TouchableOpacity style={{ flex: 1 }} onPress={() => setShowVariants(false)} />
                    <Animated.View
                    ...
                    >
                        <View className="flex-row items-center justify-between p-4 border-b border-gray-100">
                            <View>
                                <Text className="text-base font-bold text-gray-900">Choose a Pack Size</Text>
                                <Text className="text-xs text-gray-500 mt-1" numberOfLines={1}>{item.name}</Text>
                            </View>
                            <TouchableOpacity onPress={() => setShowVariants(false)} className="p-2">
                        ...
                        <ScrollView contentContainerStyle={{ padding: 16 }}>
                            {item.variants?.map((variant: any, idx: number) => {
                                ...
                                const vOldPrice = variant.old_price || variant.variant_old_price;
                                ...
                                return (
                                    ...
                                    >
                                        <View className="flex-1">
                                            <Text className="text-sm font-semibold text-gray-900 mb-1">{vWeight}</Text>
                                            <View className="flex-row items-center gap-2">
                                                <Text className="text-base font-bold text-gray-900">₹{vPrice}</Text>
                                                {vOldPrice && (
                                                    <Text className="text-xs text-gray-400 line-through">₹{vOldPrice}</Text>
                                                )}
                                                {saved > 0 && (
                                                    <View className="bg-orange-100 px-1.5 py-0.5 rounded">
                                                        <Text className="text-[10px] text-orange-700 font-bold">₹{saved} OFF</Text>
                                                    </View>
                                                )}
                                            </View>
                                        </View>
                                        ...
                                        >
                                            <Text className="text-[#FD5B00] font-bold text-xs uppercase">Add</Text>
                                        </TouchableOpacity>
                                    </TouchableOpacity>
                                )
                            })}
